...
LATEX_ENV_MATH_RE = re.compile(
    r"\\begin\{(equation\*?|align\*?|aligned|gather\*?|multline\*?|eqnarray\*?|split|cases)\}(.*?)\\end\{\1\}",
    re.DOTALL,
)
...
def export_pdf_with_browser(input_source: str, output_path: Path) -> subprocess.CompletedProcess[str]:
...
def split_markdown_target(raw_target: str) -> str:
    ...
    title_match = re.match(r'^(?P<path>.+?)\s+(?P<title>"[^"]*"|\'[^\']*\')\s*$', target)
    if title_match:
        return title_match.group("path").strip()

    simple_match = re.match(r'^(?P<path>\S+)\s*$', target)
    if simple_match:
        return simple_match.group("path").strip()
    return target


def normalize_asset_target(raw_target: str) -> str:
    target = urllib.parse.unquote(split_markdown_target(raw_target))
    if target.startswith(("http://", "https://", "data:")):
        return target

    if target.startswith("file://"):
        parsed = urllib.parse.urlparse(target)
        file_path = urllib.parse.unquote(parsed.path or "")
        if sys.platform == "win32" and re.match(r"^/[A-Za-z]:", file_path):
            file_path = file_path[1:]
        return file_path

    parsed = urllib.parse.urlparse(target)
    if parsed.scheme and len(parsed.scheme) == 1 and parsed.path.startswith(("\\", "/")):
        # Keep Windows drive-letter paths like C:\foo\bar.png intact.
        target = f"{parsed.scheme}:{parsed.path}"
    elif parsed.scheme:
        return target

    target = urllib.parse.unquote(parsed.path or target)
    target = target.strip()
...
def iter_asset_target_variants(raw_target: str) -> list[str]:
    normalized = normalize_asset_target(raw_target)
    variants: list[str] = []
...
def apply_inline_markup(text: str, doc_path: Path, render_citations: bool = True) -> str:
    ...
    text = LATEX_ENV_MATH_RE.sub(lambda m: stash_math(f"$${m.group(0)}$$", True), text)
    ...
    def replace_image(match: re.Match[str]) -> str:
        ...
        target = normalize_asset_target(match.group(2))
...
def build_asset_url(doc_path: Path, raw_target: str) -> str:
    ...
    encoded_target = urllib.parse.quote(normalize_asset_target(raw_target))
    return f"/asset?doc={encoded_doc}&target={encoded_target}"


def resolve_asset_path(doc_path: Path, raw_target: str) -> Path | None:
    target_variants = iter_asset_target_variants(raw_target)
    if any(target.startswith(("http://", "https://", "data:")) for target in target_variants):
        return None

    basenames: list[str] = []
    direct_candidates: list[Path] = []
    for normalized_target in target_variants:
        absolute_candidate = Path(normalized_target).expanduser()
        if absolute_candidate.is_absolute():
            try:
                candidate = absolute_candidate.resolve()
                if candidate.exists() and candidate.is_file():
                    return candidate
            except Exception:
                pass

        candidate = (doc_path.parent / normalized_target).resolve()
        if candidate.exists() and candidate.is_file():
            return candidate
        ...
        direct_candidates.extend(
            [
                ...
                doc_path.parent / "image" / basename,
                doc_path.parent / "assets" / basename,
                doc_path.parent / "figures" / basename,
                doc_path.parent / f"{doc_path.stem}_files" / "images" / basename,
                doc_path.parent / f"{doc_path.stem}_files" / basename,
                doc_path.parent / f"{doc_path.stem}_images" / basename,
                doc_path.parent / doc_path.stem / "images" / basename,
                doc_path.parent / doc_path.stem / basename,
                doc_path.parent / basename,
            ]
        )

    seen_candidates: set[str] = set()
    for fallback in direct_candidates:
        try:
            fallback = fallback.resolve()
        except Exception:
            continue
        fallback_str = str(fallback)
        if fallback_str in seen_candidates:
            continue
        seen_candidates.add(fallback_str)
    ...
    for basename in basenames:
        ...
        if cache_key in ASSET_SEARCH_CACHE:
            cached = ASSET_SEARCH_CACHE[cache_key]
            if cached:
                return cached
            continue
        ...
        search_roots = [
            doc_path.parent,
            doc_path.parent / doc_path.stem,
            doc_path.parent / "images",
            doc_path.parent / "assets",
            doc_path.parent / "figures",
            doc_path.parent / f"{doc_path.stem}_files",
            doc_path.parent / f"{doc_path.stem}_images",
        ]
        if sys.platform == "win32":
            search_roots.extend(
                [
...
def rewrite_html_assets(html_block: str, doc_path: Path) -> str:
    def replace_img_src(match: re.Match[str]) -> str:
        raw_src = normalize_asset_target(match.group(2))
...
def analyze_document(markdown_text: str, doc_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "block_math_count": block_math_count + len(LATEX_ENV_MATH_RE.findall(markdown_text)),
...
<head>
  ...
  <style>
    ...
    sup.citation {{ display: inline-block; position: relative; top: -0.42em; font-size: 0.68em; line-height: 1; vertical-align: baseline; margin-left: 0.08em; font-variant-numeric: tabular-nums; }}
...
class MarkdownReaderHandler(BaseHTTPRequestHandler):
    ...
    def repair_missing_assets(self) -> None:
        ...
        try:
            ...
            markdown_text = doc_path.read_text(encoding="utf-8")
            image_targets = [normalize_asset_target(target) for _, target in IMAGE_RE.findall(markdown_text)]
            image_targets.extend(normalize_asset_target(target) for _, target, _ in HTML_IMG_SRC_RE.findall(markdown_text))
    ...
    def export_pdf(self) -> None:
        ...
        try:
            ...
            if PDF_HELPER_PATH.exists():
                ...
                markdown_text = doc_path.read_text(encoding="utf-8")
                print_html = build_print_html(doc_path, markdown_text)
                with tempfile.NamedTemporaryFile("w", suffix=".html", delete=False, encoding="utf-8") as temp_html:
                    temp_html.write(print_html)
                    temp_html_path = Path(temp_html.name)
                try:
                    result = export_pdf_with_browser(temp_html_path.resolve().as_uri(), output_path)
                finally:
                    try:
